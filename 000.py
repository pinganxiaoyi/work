...
#print(f"Number of graphs in training set: {len(trainset)}")
#print(f"Number of graphs in validation set: {len(valset)}")
#print(f"Number of graphs in test set: {len(testset)}")
#--另外文件验证
dataset2 = Make_Graphs("F:\\work\\muon_10GeV_noCalo_comp_vertical-center-nX.g4mac.root")
#设置模型
...
none_count = 0
#自定义的评价函数
def resolution(model2, graph,labels, features, efeatures,label,theta,phi):
...
# Call the resolution function with the selected graph
correct, delta_xy, delta_xz, delta_yz, delta_tot_edep = resolution(model, graph, labels, graph.ndata['feat'], graph.edata['weight'], graph.edata['label'], graph.edata['theta'], graph.edata['phi'])
